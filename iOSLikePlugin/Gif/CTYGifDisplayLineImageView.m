...
@interface CTYGifDisplayLineImageView ()
{
    int tmp;
}
@property (nonatomic) NSUInteger currentFrameIndex;
@property (nonatomic,strong) CTYGifDisplayLineImage *animatedImage;
...
@end
@implementation CTYGifDisplayLineImageView
...
-(void)setImage:(UIImage *)image
{
    ...
    self.accumulator = 0;
    if ([image isKindOfClass:[CTYGifDisplayLineImage class]] && image.images) {
        ...
        self.animatedImage = (CTYGifDisplayLineImage *)image;
...
-(void)changeKeyframe:(CADisplayLink *)displayLink
{
    ...
    while (self.accumulator >= self.animatedImage.frameDurations[self.currentFrameIndex]) {
        ...
        if (++self.currentFrameIndex >= self.animatedImage.images.count-2) {//确定停下的时候是哪个帧
